/*
    ...
    * 트랜잭션
        ...
        - 데이터의 변경사항 ( DML ) 들을 하나의 트랜잭션에 묶어서 처리
            DML 문 한개를 수행할 때 트랜잭션이 존재하면 해당 트랜잭션이 같이 묶어서 처리
        ...
        - COMMIT : 한 트랜잭션에 담겨 있는 변경사항들을 실제 DB에 반영시키겠다는 의미(이후에 트랜잭션은 사라짐)
        - ROLLBACK : 한 트랜잭션에 담겨있는 변경사항들을 삭제 (취소) 한 후 마지막 COMMIT시점으로 돌아감
        - SAVEPOINT : 현재 이 시점에 해당 포인트명으로 임시저장점을 정의해 두는 것
                             ROLLBACK 진행시 전제 변경사항을 삭제하는것이 아니라 일부만 삭제 
*/
CREATE TABLE EMP_01 
...
ROLLBACK; -- COMMIT이 되었기 때문에 ROLLBACK 불가
SELECT *  FROM EMP_01;
...
SELECT * FROM EMP_01 WHERE EMP_ID = 222;
