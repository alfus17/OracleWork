...
/*
    ...
    ROLLUP / CUBE =>  GROUP BY 절에 기술
     -ROLLUP(컬럼1, 컬럼 2 ) :  컬럼1을 가지고 다시 중간집계를 내는 함수
     -CUBE(컬럼 1, 컬럼 2) : 컬럼1을 가지고 중간집계를  내고, 컬럼 2을 가지고도 중간집계를 냄
...
ORDER BY JOB_CODE;  -- 컬럼이 1개 일때 써도되지만 의미가 없다.
...
/*
    * 집합 연산자 = SET OPERATION
        ...
        - UNION ALL : 합집합 + 교집합( 중복된 값은 2번 표현될수 있따)
        - MINUS : 차집합 ( 선행 결과값에서 후행결과값을 뺀 나머지
...
FROM EMPLOYEE
WHERE SALARY>3000000;
-- 각 쿼리문의 SELECT 절에 작성되는 컬럼은 동일해야 됨
...
--------------------------------------------------------------------------3 . UNION ALL------------------------------------------------------------------------------------
-- 부서코드가 D5 또는 급여가 300만원 초과인 사원들 모두 조회(중복은 중복된 갯수만큼 조회)
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL
...
--------------------------------------------------------------------------4. MINUS------------------------------------------------------------------------------------
-- 부서코드가 D5인 사원들중 300만원 초과인 사람들을 제외하고 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
