...
--      DBMS_OUTPUT.PUT_LINE('TEAM : ' || TEAM);
    ...
    ELSE
        TEAM := '해외';
    END IF;
    DBMS_OUTPUT.PUT_LINE(TEAM || '팀 입니다');
...
DNAME := CASE DCODE
    ...
    DBMS_OUTPUT.PUT_LINE(EID || '는' || DNAME || '입니다');
END;
/
...
-------------------------------------------연습문제
/*
1.  사원의 연봉을 구하는 PL/QSL 작성, 보너스가 있는 사원은 보너스도 포함하여 계산
...
BEGIN
    ...
    IF BONUS IS NULL
        THEN  SALARYS := SALARY *12;
    ELSE
        SALARYS := (SALARY + (1+BONUS)*SALARY)*12;
    END IF;
    
    DBMS_OUTPUT.PUT_LINE('사번 : ' || EID);
    DBMS_OUTPUT.PUT_LINE('사원명 : ' || ENAME);
    DBMS_OUTPUT.PUT_LINE('연봉 : ' || SALARYS || '원' || ', 보너스 : ' || NVL(BONUS,0));
...
BEGIN
    FOR N1 IN 1..9 LOOP
        IF MOD(N1, 2) = 0 THEN
            FOR N2 IN 1..9 LOOP
                DBMS_OUTPUT.PUT_LINE(N1 || ' X ' || N2 || ' = ' || N1 * N2);
            END LOOP;
        END IF;
    END LOOP;
END;
