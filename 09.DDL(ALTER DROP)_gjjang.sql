...
ALTER TABLE DEPT_COPY
        DROP CONSTRAINT SYS_C007610
        DROP CONSTRAINT SYS_C007609;
...
*/
--DEPT_COPY 테이블의  DEPT_TITLE 컬럼명을 DEPT_NAME으로변경
...
ALTER TABLE DEPT_COPY
    ...
    ADD CONSTRAINT UQ UNIQUE (DEPT_NAME)
    MODIFY LNAME CONSTRAINT NN NOT NULL;

ALTER TABLE DEPT_COPY
    RENAME CONSTRAINT UQ  TO CP_UQ;
...
/*
    -테이블 삭제시 외래키의 부모 테이블은 삭제 안됨
        그래도 삭제하고 싶다면
        * 방법 1 : 자식테이블 먼저 삭제한 후 부모테이블 삭제
        * 방법 2 : 부모 테이블만 삭제하는 제약조건 같이 삭제하는 방법
