/*
    ...
    조회 결과는 하나의 결과물( RESULT SET)로 나옴
        ...
        JOIN 은 크게  "오라클 전용 구문 " 과  "ANSI구문" (ANSI == 미국국립표준협회)
...
/*
    >> 오라클 전용 구문
        - FROM 절에 죄회하고자 하는 테이블들을 나열( , 구분자로 사용)
        - WHERE 절에 매칭시킬 컬럼(연결고리) 에 대한 조건 제시함
        - 
*/
-- 연결할 컬럼명이 다른 경우 (EMPLOYEE에서는 DEPT_CODE , DEPARTMENT : DEPT_ID)
-- 사번, 사원명, 부서코드, 부서명을 조회
...
/*
    >> ANSI 구문
        - FROM 절에 기준이되는 테이블을 하나만 기술
        - JOIN 절에  같이 조회하고자 하는 테이블 기술 + 매칭시킬 컬럼에 대한 기술
        - JOIN USING , JOIN ON 
...
-- 2) 연결한 컬럼명이 같은 경우( EMPLOYEE : JOB_CODE, JOB: JOB_CODE)
--        -> 오로지 JOIN ON 과 JOIN USING 둘 다 사용가능
...
FROM EMPLOYEE
JOIN JOB USING ( JOB_CODE); --하나만 쓸때는 USING
...
SELECT EMP_ID , EMP_NAME, EMPLOYEE.JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB ON (EMPLOYEE.JOB_CODE= JOB.JOB_CODE); --하나만 쓸때는 USING
--JOIN USING 두 테이블의 컬럼명이 같은경우 1개만 기술
...
/*
    4. 자체 조인
        : 같은 테이블을 다시 한번 조인 하는 경우
*/
-- 전체사원의 사원번호, 사원명, 부서코드, 사수번호, 사수명, 사수부서코드
-- >> 오라클 전용 구문
-- 사수가 있는 사원만 조회
SELECT E.EMP_ID , E.EMP_NAME , E.DEPT_CODE , E.MANAGER_ID 담당사수아이디,
            M.EMP_ID 사수번호 , M.EMP_NAME  사수이름, M.DEPT_CODE 사수코드
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE , JOB_NAME
FROM EMPLOYEE E, DEPARTMENT D , JOB J
JOIN DEPARTMENT ON (DEPT_CODE = DEPT)
JOIN JOB USING(JOB_CODE);


--사번, 사원명, 부서명, 지역명 조회
-- EMPLOYEE         DEPT_CODE
-- DEPARTMENT    DEPT_ID         LOCAL_ID
-- LOCATION                             LOCAL_CODE
...
--                     DEPT_CODE                                                                         EMP_NAME
--                     DEPT_ID           LOCATION_ID                                                DEPT_TITLE
...
JOIN 
    LOCATION L  ON(LOCATION_ID = LOCAL_CODE)
JOIN  
